                        ...
                        <form action="{{ route('backend.transaksi.update', $transaksi->id) }}" method="POST"
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="jenis" id="jenis"
                                        ...
                                        <option value="pengeluaran"
                                            {{ $transaksi->jenis == 'pengeluaran' ? 'selected' : '' }}>Pengeluaran</option>
                                    </select>
                                    @error('jenis')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="keterangan" id="keterangan" rows="3"
                                    class="form-control @error('keterangan') is-invalid @enderror" placeholder="Tulis keterangan transaksi...">{{ $transaksi->keterangan }}</textarea>
                                @error('keterangan')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
